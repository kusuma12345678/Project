        ...
        <form action="" method="post">
            ...
            <legend>Enter Marks</legend>
                ...
                <?php
                    ...
                    $select_semester_query="SELECT `name` from `semester`";
                    $semester_result=mysqli_query($conn,$select_semester_query);
                    //select semester
                    echo '<select name="semester_name">';
                    echo '<option selected disabled>Select Semester</option>';
                    
                        while($row = mysqli_fetch_array($semester_result)) {
                            $display=$row['name'];
...
<?php
    if(isset($_POST['rno'],$_POST['p1'],$_POST['p2'],$_POST['p3'],$_POST['p4'],$_POST['p5']))
    {
        ...
        // validation
        if (empty($semester_name) or empty($rno) or $p1>100 or  $p2>100 or $p3>100 or $p4>100 or $p5>100 or $p1<0 or  $p2<0 or $p3<0 or $p4<0 or $p5<0 ) {
        ...
        $name=mysqli_query($conn,"SELECT `name` FROM `students` WHERE `rno`='$rno' and `semester_name`='$semester_name'");
        while($row = mysqli_fetch_array($name)) {
            $display=$row['name'];
            echo $display;
         }

        $sql="INSERT INTO `result` (`name`, `rno`, `semester`, `p1`, `p2`, `p3`, `p4`, `p5`, `marks`, `percentage`) VALUES ('$display', '$rno', '$semester_name', '$p1', '$p2', '$p3', '$p4', '$p5', '$marks', '$percentage')";
